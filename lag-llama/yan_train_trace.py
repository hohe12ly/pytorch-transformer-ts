...
# YL: get_dataset creates 3 things: metadata.json, train/data.json, test/data.json
# YL: metadata.json contains freq, prediction_length, cardinality, target, feat_static_cat, feat_static_real, feat_dynamic_cat, feat_dynamic_real
# YL: train/data.json and test/data.json contain target = [target sequence], start, item_id, feat_static_cat
...
# #estimator args
seed = 0 # parser.add_argument("--seed", type=int, default=0)
context_length = 256 # parser.add_argument("--context_length", type=int, default=256)
n_layer = 4 # parser.add_argument("--n_layer", type=int, default=4)
n_embd = 256 # parser.add_argument("--n_embd", type=int, default=256)
n_head = 4 # parser.add_argument("--n_head", type=int, default=4)
aug_prob = 0.5 # parser.add_argument("--aug_prob", type=float, default=0.5)
aug_rate = 0.1 # parser.add_argument("--aug_rate", type=float, default=0.1)
batch_size = 100 # parser.add_argument("--batch_size", type=int, default=100)
num_batches_per_epoch = 100 # parser.add_argument("--num_batches_per_epoch", type=int, default=100)
# # estimator trainer kwarg args
limit_val_batches = 10 # parser.add_argument("--limit_val_batches", type=int, default=10)
max_epochs = 1000 # parser.add_argument("--max_epochs", type=int, default=1000)
# #parser.add_argument("--gpu", type=int, default=0)
# # Other args
test = False # parser.add_argument('--test', action='store_true')
early_stopping_patience = 50 # parser.add_argument('--early_stopping_patience', default=50)
# # Model
...
early_stop_callback = EarlyStopping(monitor="val_loss", min_delta=0.00, patience=int(early_stopping_patience), verbose=True, mode="min")
...
for name in psml_dataset_names[4:5]: # PSML case 2, Vm 
    ...
    agg_metrics["n_layer"] = n_layer
    agg_metrics["n_embd"] = n_embd
    agg_metrics["n_head"] = n_head
...
# DEBUG: vis test results
import json
log_dir = '/mnt/data/home/yxl/test/lagllama/pytorch-transformer-ts.kashif/lag-llama/model-size-scaling-logs.lagllama_3GPUs/0/experiments/lightning_logs/version_0'
perf_file = f'{log_dir}/m4_weekly.json'
